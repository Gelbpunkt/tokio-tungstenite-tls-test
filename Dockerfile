...
RUN rm -f target/$RUST_TARGET/release/deps/tokio_tungstenite_tls_test*
COPY ./src ./src

RUN source $HOME/.cargo/env && \
    cargo build --release \
        --target="$RUST_TARGET" && \
    cp target/$RUST_TARGET/release/tokio-tungstenite-tls-test /tokio-tungstenite-tls-test && \
    actual-strip /tokio-tungstenite-tls-test

FROM scratch

COPY --from=builder /tokio-tungstenite-tls-test /tokio-tungstenite-tls-test

CMD ["./tokio-tungstenite-tls-test"]
